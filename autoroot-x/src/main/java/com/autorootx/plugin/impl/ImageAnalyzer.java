...
public class ImageAnalyzer implements Analyzer {
    ...
    public AnalysisResult analyze(AnalysisRequest req) {
        ...
        String vulnSummary = buildVulnSummary(image, vulns,
                trivyAvailable ? "Trivy sidecar scan" : "Fallback static CVE dataset (Trivy unavailable)");
    ...
    private Optional<TrivyService.TrivyScanResult> parsePayloadTrivyReport(Map<String, Object> payload) {
        ...
        try {
            ...
            return Optional.of(new TrivyService.TrivyScanResult(
                    true,
                    parseTrivyVulnerabilities(root),
                    String.valueOf(raw),
                    "Trivy report supplied in request payload"
            ));
        } catch (Exception e) {
            return Optional.of(new TrivyService.TrivyScanResult(
                    true,
                    List.of(),
                    String.valueOf(raw),
                    "Unable to parse supplied Trivy report: " + e.getMessage()
            ));
        }
    }
